...
/usr/local/bin/python scripts/migrations/populate_points_for_h3.py --config ./isb.cfg >& /var/log/isamples/`date "+%Y-%m-%d"`.sesar_points.txt
echo "Going to fetch heights for any points without height values"
node elevate.js -d "postgresql+psycopg2://isb_writer:isamplesinabox@db/isb_1" -k "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiI4MzRhMWQ2MS04YjQ2LTQxY2YtYjAwMi1mYWQ5YTcyYjE4MzQiLCJpZCI6OTA5NDAsImlhdCI6MTY1MDY0OTQ3MH0.9SO-QGYpjXAOYjj52guTmFJsxRGREekFADUw19hSKB0" >& /var/log/isamples/`date "+%Y-%m-%d"`.sesar_elevate.txt
